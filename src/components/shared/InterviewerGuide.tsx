...
import { INTERVIEW_GUIDE_STEPS, withInterviewPath } from '../../lib/interviewPath';
  ...
  return (
    ...
    >
      ...
      <p className="mb-5 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
        ...
        <Link to="/contact" className="font-semibold text-violet-600 hover:underline dark:text-violet-400">
      ...
      <ol className="grid gap-3 sm:grid-cols-3">
        {INTERVIEW_GUIDE_STEPS.map((step, i) => {
          ...
          return (
            <li key={step.id}>
              <Link
                to={withInterviewPath(step.path)}
